...
func (m *Upstream) Hash(hasher hash.Hash64) (uint64, error) {
	...
	case *Upstream_AwsTg:
		...
		if h, ok := interface{}(m.GetAwsTg()).(safe_hasher.SafeHasher); ok {
			if _, err = hasher.Write([]byte("AwsTg")); err != nil {
				return 0, err
			}
			if _, err = h.Hash(hasher); err != nil {
				return 0, err
			...
			if fieldValue, err := hashstructure.Hash(m.GetAwsTg(), nil); err != nil {
				return 0, err
			} else {
				if _, err = hasher.Write([]byte("AwsTg")); err != nil {
					return 0, err
