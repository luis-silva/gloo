...
type Global struct {
	...
	ExtauthCustomYaml     *bool            `json:"extauthCustomYaml,omitempty" desc:"Inject whatever yaml exists in .Values.global.extauthCustomYamlBody or .Values.global.extensions.extAuth into settings.spec.extauth, instead of structured yaml (which is enterprise only). Defaults to true in open source, and false in enterprise"`
	ExtauthCustomYamlBody interface{}      `json:"ExtauthCustomYamlBody,omitempty" desc:"Generic yaml to be inserted for third party server support"`
}
